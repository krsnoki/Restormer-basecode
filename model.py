...
class MDTA(nn.Module): 
    ...
    def forward(self, x):
        ...
        q = q.reshape(b, self.num_heads, -1, h * w) # R
        k = k.reshape(b, self.num_heads, -1, h * w) # R
        v = v.reshape(b, self.num_heads, -1, h * w) # R
        q, k = F.normalize(q, dim=-1), F.normalize(k, dim=-1)

        attn = torch.softmax(torch.matmul(q, k.transpose(-2, -1).contiguous()) * self.temperature, dim=-1) # k transposed 
